...
fn main() {
  ...
  let start_btn 	= gloo::utils::body().query_selector("#start-btn").unwrap().unwrap();
  let result_div	= gloo::utils::body().query_selector("#result").unwrap().unwrap();
  let started   	= Rc::new(Cell::new(false));
  let (bridge_sink, mut bridge_stream) = Prime::spawner().spawn("/example_prime_worker.js").split();
  ...
  spawn_local(async move {
    ...
    loop { sleep(Duration::from_secs(3600)).await; }// We create a loop so that listeners can be held for forever.
  });
}
